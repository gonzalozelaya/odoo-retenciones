    ...
    for template_code in used_template_codes:
        ...
        for company in ar_companies.filtered(lambda c: c.chart_template == template_code):
            if company.parent_id:
                _logger.info("Company %s is a branch, skipping certain operations...", company.name)
                continue  # Saltar operaciones para sucursales
            else:
                _logger.info("Company %s does not have a parent, continuing with operations...", company.name)
